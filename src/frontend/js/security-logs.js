document.addEventListener("DOMContentLoaded", () => {
  ...
  const renderLogs = (rows) => {
    ...
    rows.forEach((l) => {
      ...
      const ts =
        l && l.timestamp ? new Date(l.timestamp).toLocaleString() : "-";
      ...
      const user =
        l && l.userFullName ? `${l.userFullName} (@${l.userUsername})` : "-";
      const emp =
        l && l.employeeFullName
          ? `${l.employeeFullName} (@${l.employeeUsername})`
          : "-";
  ...
  const loadAndRenderLogs = async () => {
    try {
      ...
      if (
        employeeSession &&
        typeof employeeSession.token === "string" &&
        employeeSession.token.trim() !== ""
      ) {
        headers.Authorization = `Bearer ${employeeSession.token}`;
      } else if (!state.demoNoticeShown) {
        showToast(
          "Demo Mode: Security logs are publicly accessible for testing",
          "info",
          5000,
        );
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        showToast(
          "Security logs unavailable. Please try again later.",
          "error",
        );
        return;
      }
  ...
  const bindControls = () => {
    ...
    if (pageSizeSel) {
      pageSizeSel.addEventListener("change", async () => {
        ...
        state.pageSize =
          Number.isFinite(nextSize) && nextSize > 0 ? nextSize : 10;
        state.currentPage = 1;
        await loadAndRenderLogs();
      });
    }
